<template>
  <div>
    ...
    >
      ...
      <div v-else :class="$style.modalBox">
        <el-form
          ...
          :class="$style.form"
        >
          <el-form-item label="环境选择" prop="envKind" required :class="$style.itemBox">
            <el-radio-group :model-value="model.envKind" @change="setModelEnvKind">
              <el-radio
          ...
          <el-form-item label="导出方式" prop="method" required :class="$style.itemBox">
            <el-radio-group :disabled="isExporting" :model-value="model.method" @change="setModelMethod">
          ...
          <template v-if="model.method === ExportMethod.COMPILED_CODE">
            <el-form-item label="制品形式" required :class="$style.itemBox">
              <el-radio-group :disabled="isExporting" :model-value="model.exportKind" @change="setModelExportKind">
          ...
          <template v-if="model.method === ExportMethod.SOURCE_CODE">
            <el-form-item label="源码形式" required :class="[$style.itemBox, $style.sourceOptions]">
              <el-radio-group :disabled="isExporting" :model-value="model.exportKindSrc" @change="setModelExportKindSrc">
                <div
                ...
                >
                  <el-radio
                    ...
                    :style="{ fontWeight: 500, color: '#1d2129', height: '24px' }"
                  >仅前端源码</el-radio>
                ...
                <div
                  v-if="!isPureFeMode"
                  :class="$style.sourceOptionWrap"
                  :active="model.exportKindSrc === ExportType.BOTH_FRONTED_BACKEND"
                  @click="handleSelectSourceOption(ExportType.BOTH_FRONTED_BACKEND)"
                >
                  <el-radio
                    :value="ExportType.BOTH_FRONTED_BACKEND"
                    :class="$style.sourceOptionRadio"
                ...
                <div
                  v-if="!isPureFeMode"
                  :class="$style.sourceOptionWrap"
                  :active="model.exportKindSrc === ExportType.BOTH_FRONTED_BACKEND_SRC"
                  @click="handleSelectSourceOption(ExportType.BOTH_FRONTED_BACKEND_SRC)"
                >
                  <el-radio
                    :value="ExportType.BOTH_FRONTED_BACKEND_SRC"
                    :class="$style.sourceOptionRadio"
            ...
            <div :class="$style.appendBox">
              <el-form-item v-if="canUploadCodeBase" style="margin-bottom: 6px">
                <el-checkbox :model-value="uploadCodeBase" @change="setUploadCodeBase">生成源码后自动上传到代码仓库</el-checkbox>
              </el-form-item>
              <el-form-item v-if="!isSourceFe" style="margin-bottom: 6px">
                <el-checkbox :model-value="model.includeJar" @change="setModelIncludeJar">
                  导出后端源码依赖的非公开JAR包
                  <el-tooltip trigger="hover" placement="right" :show-after="500">
              ...
              <el-form-item v-if="!!lastVersion && !isSourceFe" style="margin-bottom: 6px">
                <el-checkbox :model-value="isComparedBackupVersion" @change="setIsComparedBackupVersion">导出增量SQL</el-checkbox>
                <div :class="$style.comparedIncrementConfigWrap">
                  <el-checkbox :model-value="isComparedIncrementConfig" @change="setIsComparedIncrementConfig">导出增量配置信息</el-checkbox>
                </div>
                <div
                  v-if="isComparedBackupVersion || isComparedIncrementConfig"
                  :class="$style.comparedBackupVersionWrap"
          ...
          <el-form-item
            ...
            :class="$style.itemBox"
          >
            <el-radio-group :model-value="model.separation" @change="setModelSeparation" :disabled="isExporting">
              <el-radio
                :value="item.value"
                v-for="item in [
            ...
            <p v-if="model.separation === false">注：前端源码作为后端源码子目录存放</p>
            <p v-if="model.separation === true">注：前后端源码按独立目录存放</p>
          </el-form-item>

          <div v-if="uploadCodeBase" :class="$style.appendBox" style="padding-bottom: 8px">
            <el-form-item label="仓库地址">
              <div v-if="model.changeSplitFloder" :class="$style.gitTitle">前端仓库：</div>
              <div style="display: flex; align-items: center">
                ...
                <span v-else>{{ model.repoProject }}</span>.git
              </div>
              ...
              <template v-if="model.changeSplitFloder">
                ...
                <div style="display: flex; align-items: center">
                  ...
                  <span v-else>{{ model.repoProjectBackend }}</span>.git
                </div>
                <div style="display: flex; margin-left: 16px; align-items: center">
                  上传分支
                  <el-input
            ...
            <div :class="$style.gitExtra">
              <el-checkbox v-if="model.separation === true" :model-value="model.changeSplitFloder" @change="setModelChangeSplitFloder">拆分源码仓库</el-checkbox>
              <el-checkbox :model-value="model.changeRepoAddress" @change="setModelChangeRepoAddress">自定义仓库地址</el-checkbox>
              <el-checkbox :model-value="model.changeRepoBranch" @change="setModelChangeRepoBranch">自定义分支名称</el-checkbox>
              <skeleton-render
          ...
          <el-form-item
            label="后端代码构建方式"
            v-if="
              !isSourceFe && showBackCodeConstructMethod && model.method === ExportMethod.SOURCE_CODE
            "
            required
            :class="$style.itemBox"
          >
            ...
            >
              <s-others-icon name="info" :class="[$style.iconInfo, $style.backendCodeConstructIcon]"></s-others-icon>
            </el-tooltip>
            <el-radio-group :model-value="constructMethod" @change="setConstructMethod" :disabled="isExporting">
              <el-radio
                :value="item.value"
          ...
          <el-form-item style="margin-bottom: 20px">
            <el-checkbox :disabled="!canBackup" :model-value="model.backupSwitch" @change="setModelBackupSwitch">应用导出成功后自动生成备份记录</el-checkbox>
          ...
          >
            <el-input type="textarea" required :rows="3" :model-value="model.commitMessage" @input="(val) => setModelCommitMessage(val)"></el-input>
          </el-form-item>
          <skeleton-render
    ...
    >
      ...
      <p v-if="finishDescription?.length > 0" style="margin: 8px 0 0 0; word-break: break-all">
        {{ finishDescription }}
      </p>
      <p v-if="finishDescription2?.length > 0" style="margin: 8px 0 0 0; word-break: break-all">
        {{ finishDescription2 }}
      </p>
      <template #footer>
        <div :style="{
          display: 'flex',
          justifyContent: submitSuccess && useSourceModeAndCodeBase ? 'space-between' : 'flex-end',
        }">
          <template v-if="submitSuccess">
            ...
            <el-button type="primary" style="margin-left: 20px" @click="closeFinishModal">完 成</el-button>
          </template>
          <template v-else>
            <el-button @click="closeFinishModal">关 闭</el-button>
            <el-button type="primary" style="margin-left: 20px" @click="reExport">重新导出</el-button>
          </template>
        </div>
      </template>
    </el-dialog>
...
.comparedBackupVersionWrap :global(.el-form-item.is-error) + .backupVersionHint {
  margin-top: 20px;
}
